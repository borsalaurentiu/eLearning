<?php {
	session_start();
	if ( empty( $_SESSION['member_type'] ) || $_SESSION['member_type'] != 'administrator' ) {
    ...
    <body>
        <div class="center">
            ...
            <img class="logo" src="../images/utcn_logo.png">
				<form action="adaugaProfesor.php" method="POST">
					<center><table>
					<tr>
						<td> Username: </td>
					...
					<input type="submit" value="Adauga profesor"></center>
...
if ($result->num_rows == 0) {	
	$sql = "INSERT INTO teachers (username, nume, prenume, materie, email, telefon) 
			VALUES ('".$_POST['username']."', '".$_POST['nume']."', '".$_POST['prenume']."',
			'".$_POST['materie']."', '".$_POST['email']."', '".$_POST['telefon']."');";
	$sqll = "INSERT INTO members (username, password, type) VALUES ('".$_POST['username']."', '".$_POST['password']."', 'profesor');";
	...
	if ($conn->query($sql) === TRUE && $conn->query($sqll) === TRUE) {
		...
		$conn->close();
    header("Location: admin.php?profesor=404");
	}
} else {
	$conn->close();
    header("Location: admin.php?profesor=404");
